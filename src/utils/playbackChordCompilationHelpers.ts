...
interface CompileFullPlaybackTab {
  tabData: Section[];
...
function compileFullPlaybackTab({
  tabData,
  sectionProgression,
  chords,
  baselineBpm,
  ...
  endLoopIndex,
  atomicallyUpdateAudioMetadata,
}: CompileFullPlaybackTab) {

  // ah maybe need to make new types for this to be similiar-ish to Section interface? 
  ...
  return {
    data: compiledChordsMappedToLoopRange,
    metadata: metadataMappedToLoopRange,
  };
}

interface CompileSpecificPlaybackChordGrouping {
  tabData: Section[];
  location: {
    sectionIndex: number;
...
function compileSpecificPlaybackChordGrouping({
  tabData,
  location,
  chords,
  baselineBpm,
  ...
  endLoopIndex,
  atomicallyUpdateAudioMetadata,
}: CompileSpecificPlaybackChordGrouping) {
  const compiledChords: string[][] = [];
  const metadata: Metadata[] = [];
  const elapsedSeconds = { value: 0 }; // getting around pass by value/reference issues, prob want to combine all three into one obj};
