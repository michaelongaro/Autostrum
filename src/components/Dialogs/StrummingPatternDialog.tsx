...
function StrummingPatternDialog() {
  ...
  // Retain last value so DialogContent can animate out after close.
  const strummingPatternRef = useRef<{
    index: number;
    value: StrummingPatternType;
  } | null>(strummingPatternBeingEdited);
  if (strummingPatternBeingEdited !== null) {
    strummingPatternRef.current = strummingPatternBeingEdited;
  }
  const strummingPattern =
    strummingPatternBeingEdited ?? strummingPatternRef.current;

  function handleBaseNoteLengthChange(
    newNoteLength: "whole" | "half" | "quarter" | "eighth" | "sixteenth",
  ) {
    if (!strummingPatternBeingEdited) return;
          ...
          for (const [subSectionIndex, subSection] of section.data.entries()) {
            if (subSection?.type === "chord") {
              for (const [
                ...
                const pattern = chordSequence?.strummingPattern;
                if (
                  !chordProgression ||
                  !pattern ||
                  pattern.id !==
    ...
    <Dialog
      onOpenChange={(open) => {
        if (!open) {
          ...
          setStrummingPatternBeingEdited(null);
          setEditingPalmMuteNodes(false);
          setShowingDeleteStrumsButtons(false);
          setLastModifiedPalmMuteNode(null);
        }
      }}
      open={strummingPatternBeingEdited !== null}
    >
      <VisuallyHidden>
        <DialogTitle>Strumming pattern editor dialog</DialogTitle>
        <DialogDescription>
          Edit the note length, palm mutes, and strums of a strumming pattern.
        </DialogDescription>
      </VisuallyHidden>
          ...
          <>
            ...
            <div className="baseVertFlex max-h-[35vh] !items-start !justify-start">
              ...
              >
                ...
                <StrummingPattern
                  data={strummingPattern.value}
                  mode={"editingStrummingPattern"}
                  index={strummingPattern.index}
            ...
            <div className="baseFlex mt-4 w-full !justify-between gap-8">
              <Button
                disabled={
                  ...
                  strummingPattern.value.strums.every(
                    (strum) => strum.strum === "",
                  )
                }
                variant={"audio"}
              ...
              <Button
                disabled={
                  ...
                  isEqual(
                    ...
                    strummingPatterns[strummingPattern.index],
                  )
                }
...
export default StrummingPatternDialog;
