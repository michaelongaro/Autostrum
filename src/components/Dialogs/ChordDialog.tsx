import isEqual from "lodash.isequal";
import { useRef } from "react";
import { BsFillPlayFill, BsKeyboard } from "react-icons/bs";
import { Label } from "~/components/ui/label";
import { useTabStore, type Chord as ChordType } from "~/stores/TabStore";
import Chord from "../Tab/Chord";
import { Button } from "~/components/ui/button";
import { Input } from "~/components/ui/input";
import { getOrdinalSuffix } from "~/utils/getOrdinalSuffix";
import { X } from "lucide-react";
import {
  CHORD_COLORS,
  getContrastTextColor,
  getColorForChordName,
} from "~/utils/chordColors";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogTitle,
} from "~/components/ui/dialog";
import { VisuallyHidden } from "@radix-ui/react-visually-hidden";

function ChordDialog() {
  const {
    chords,
    setChords,
    chordBeingEdited,
    setChordBeingEdited,
    audioMetadata,
    previewMetadata,
    capo,
    playPreview,
    pauseAudio,
    setTabData,
  } = useTabStore((state) => ({
    chords: state.chords,
    setChords: state.setChords,
    chordBeingEdited: state.chordBeingEdited,
    setChordBeingEdited: state.setChordBeingEdited,
    audioMetadata: state.audioMetadata,
    previewMetadata: state.previewMetadata,
    capo: state.capo,
    playPreview: state.playPreview,
    pauseAudio: state.pauseAudio,
    setTabData: state.setTabData,
  }));

  // Retain last value so DialogContent can animate out after close.
  const chordRef = useRef<{ index: number; value: ChordType } | null>(
    chordBeingEdited,
  );
  if (chordBeingEdited !== null) {
    chordRef.current = chordBeingEdited;
  }
  const chord = chordBeingEdited ?? chordRef.current;

  function handleChordNameChange(e: React.ChangeEvent<HTMLInputElement>) {
    if (!chordBeingEdited) return;

    const value = e.target.value;

    if (value.length > 16) return;

    const modifiedChord = structuredClone(chordBeingEdited);
    modifiedChord.value.name = value;

    // Auto-assign color based on chord name
    const autoColor = getColorForChordName(value);
    if (autoColor) {
      modifiedChord.value.color = autoColor;
    }

    setChordBeingEdited(modifiedChord);
  }

  function handleColorChange(color: string) {
    if (!chordBeingEdited) return;

    const modifiedChord = structuredClone(chordBeingEdited);
    modifiedChord.value.color = color;
    setChordBeingEdited(modifiedChord);
  }

  function handleSaveChord() {
    if (!chordBeingEdited) return;

    const modifiedChord = structuredClone(chordBeingEdited);

    // if the name already exists on a different chord, append " (2)", " (3)", etc.
    const nameConflict = chords.some(
      (existingChord, index) =>
        existingChord.name === modifiedChord.value.name &&
        index !== modifiedChord.index,
    );

    if (nameConflict) {
      const baseName = modifiedChord.value.name;
      let suffix = 2;
      let candidate = `${baseName} (${suffix})`;
      while (chords.some((c) => c.name === candidate)) {
        suffix += 1;
        candidate = `${baseName} (${suffix})`;
      }
      modifiedChord.value.name = candidate;
    }

    // update chord name of all strumming patterns that use this chord
    const oldChordName = chords[modifiedChord.index]?.name;
    if (oldChordName && oldChordName !== modifiedChord.value.name) {
      setTabData((draft) => {
        for (const [sectionIndex, section] of draft.entries()) {
          if (!section) continue;

          for (const [subSectionIndex, subSection] of section.data.entries()) {
            if (subSection?.type !== "chord") continue;

            for (const [
              chordSequenceIndex,
              chordGroup,
            ] of subSection.data.entries()) {
              if (!chordGroup) continue;

              for (const [chordIndex, chordName] of chordGroup.data.entries()) {
                if (!chordName || chordName !== oldChordName) continue;

                const sectionData = draft[sectionIndex];
                if (!sectionData) continue;

                const subSectionData = sectionData.data[subSectionIndex];
                if (subSectionData?.type !== "chord") continue;

                const chordSequence = subSectionData.data[chordSequenceIndex];
                if (!chordSequence) continue;

                chordSequence.data[chordIndex] = modifiedChord.value.name;
              }
            }
          }
        }
      });
    }

    // save chord
    const newChords = structuredClone(chords);

    // decomposed shallow copy of frets so that the chord elem won't get updated
    // when the chord is edited in the chord dialog
    const newChord = structuredClone(modifiedChord.value);
    newChords[modifiedChord.index] = {
      id: newChord.id,
      name: newChord.name,
      frets: [...newChord.frets],
      color: newChord.color,
    };
    if (audioMetadata.playing) pauseAudio();
    setChordBeingEdited(null);
    setChords(newChords);
  }

  return (
    <Dialog
      onOpenChange={(open) => {
        if (!open) {
          if (audioMetadata.playing) pauseAudio();
          setChordBeingEdited(null);
        }
      }}
      open={chordBeingEdited !== null}
    >
      <VisuallyHidden>
        <DialogTitle>Chord editor dialog</DialogTitle>
        <DialogDescription>
          Edit the name, color, and fretting of a chord.
        </DialogDescription>
      </VisuallyHidden>

      <DialogContent className="baseVertFlex min-w-[350px] max-w-[90vw] gap-4 rounded-lg border p-4 shadow-sm xs:w-[380px] xs:gap-8">
        {chord && (
          <>
            {/* chord title */}
            <div className="baseFlex w-full !items-start !justify-between">
              <div className="baseVertFlex !items-start gap-2">
                <div className="baseFlex gap-2">
                  <Label htmlFor="chordName">Chord name</Label>
                  <Input
                    id="chordName"
                    placeholder="(e.g. Em, Cmaj7)"
                    value={chord.value.name}
                    onChange={handleChordNameChange}
                    className="w-[150px]"
                  />
                </div>

                <div className="baseFlex mt-1 !items-start gap-3">
                  <Label className="mt-1">Chord color</Label>

                  <div className="z-10 grid grid-cols-6 gap-2">
                    {CHORD_COLORS.map((color) => (
                      <button
                        key={color}
                        type="button"
                        onClick={() => handleColorChange(color)}
                        style={{
                          backgroundColor: color,
                          color: getContrastTextColor(color),
                        }}
                        className={`baseFlex size-7 rounded-full transition-all ${
                          chord.value.color === color
                            ? "ring-1 ring-primary ring-offset-2 ring-offset-background"
                            : "hover:scale-110"
                        }`}
                      >
                        {chord.value.color === color && (
                          <svg
                            className="size-4"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                            strokeWidth={3}
                          >
                            <path
                              strokeLinecap="round"
                              strokeLinejoin="round"
                              d="M5 13l4 4L19 7"
                            />
                          </svg>
                        )}
                      </button>
                    ))}
                  </div>
                </div>
              </div>

              <Button
                variant={"modalClose"}
                onClick={() => {
                  if (audioMetadata.playing) pauseAudio();
                  setChordBeingEdited(null);
                }}
              >
                <X className="size-5" />
                <span className="sr-only">Close</span>
              </Button>
            </div>

            <div className="baseVertFlex my-2 w-full gap-2 sm:my-1">
              <div className="baseFlex w-full !items-start !justify-between gap-4">
                <Chord
                  chordBeingEdited={chord}
                  highlightChord={
                    previewMetadata.indexOfPattern === chord.index &&
                    previewMetadata.playing &&
                    previewMetadata.type === "chord"
                  }
                />

                {/* ml here seems hacky */}
                <div className="baseVertFlex h-full w-48 gap-4">
                  <div className="baseVertFlex w-full !items-start gap-2 rounded-lg border bg-secondary p-2 text-sm shadow-sm">
                    <div className="baseFlex w-[160px] gap-2 font-semibold">
                      <BsKeyboard className="h-6 w-6" />
                      Hotkeys
                    </div>

                    <div className="grid w-full grid-cols-[45px_5px_90px] gap-2">
                      <div className="baseFlex gap-1">
                        <kbd>A</kbd> - <kbd>G</kbd>
                      </div>
                      <span>-</span>
                      <span>Major chords</span>

                      <div className="baseFlex gap-1">
                        <kbd>a</kbd> - <kbd>g</kbd>
                      </div>
                      <span>-</span>
                      <span>Minor chords</span>

                      <kbd className="h-min w-min place-self-end">x</kbd>
                      <span>-</span>
                      <span>Muted string</span>
                    </div>
                  </div>

                  {capo > 0 && (
                    <div className="baseVertFlex !items-start gap-1 text-sm">
                      <p className="font-medium underline underline-offset-2">
                        Reminder
                      </p>
                      <p>
                        Fret values should be relative to capo on the{" "}
                        {getOrdinalSuffix(capo)} fret.
                      </p>
                    </div>
                  )}
                </div>
              </div>
            </div>

            {/* a bit janky to have absolute positioning on this  */}
            <div className="baseFlex w-full !justify-between gap-4">
              <Button
                disabled={
                  (previewMetadata.indexOfPattern === chord.index &&
                    previewMetadata.playing &&
                    previewMetadata.type === "chord") ||
                  chord.value.frets.every((fret) => fret === "")
                }
                variant={"audio"}
                className="baseFlex gap-2"
                onClick={() => {
                  void playPreview({
                    data: chord.value.frets,
                    index: chord.index,
                    type: "chord",
                  });
                }}
              >
                <BsFillPlayFill className="h-6 w-6" />
                Preview
              </Button>

              <Button
                disabled={
                  chord.value.frets.every((fret) => fret === "") ||
                  chord.value.name === "" ||
                  isEqual(chord.value, chords[chord.index])
                }
                onClick={handleSaveChord}
                className="px-12"
              >
                Save
              </Button>
            </div>
          </>
        )}
      </DialogContent>
    </Dialog>
  );
}

export default ChordDialog;
