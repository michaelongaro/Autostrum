...
function EditImageSelector({
  ...
  return (
    <div className="baseVertFlex relative w-full !items-start gap-4 lg:!flex-row lg:!justify-between">
              ...
              <AnimatePresence>
                {!profileImageLoaded && (
                  <motion.div
                    ...
                    className="pulseAnimation absolute inset-0 z-10 size-16 rounded-full lg:size-32"
                  ></motion.div>
                )}
              </AnimatePresence>
            </>
          ) : (
            <div className="pulseAnimation size-16 rounded-full shadow-sm lg:size-32"></div>
          )}
        </motion.div>
