...
function PlaybackMenuContent() {
  ...
  // Return a single element (not a Fragment) so AnimatePresence/popLayout can
  // safely attach refs without hitting React.Fragment prop restrictions.
  if (playbackModalViewingState === "Section progression") {
    return (
      <motion.div
  ...
  if (playbackModalViewingState === "Chords") {
    return (
      <motion.div
        key="Chords"
        initial={{ opacity: 0 }}
  ...
  if (playbackModalViewingState === "Strumming patterns") {
    return (
      <motion.div
