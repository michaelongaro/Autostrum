> Historical decision notes. Canonical domain language lives in [`CONTEXT.md`](../CONTEXT.md); hard-to-reverse decisions are recorded in [`docs/adr/`](../docs/adr/).
...
- Screenshots are now taken on the dedicated Hono screenshot server due to the fact that it took upwards of 10 seconds to create/update a tab. The extra logistical work on the backend is more than worth making the user wait for that long. The bulk screenshot script is still run locally and is especially useful for when changes to the static version of the tab are made, so every screenshot can be updated easily.
...
**Virtualization/Infinite looping strategy**  
Previously I used an inefficient method of virtualizing the positions of the expanded tabs, where I would check on every chord that was played which chords were out of view, and then adjust their positions to be shifted by one whole tab width. What I currently am doing is using is only adjusting the chord positions (it's acutually just a separate array of loopCount values so that I get to keep the original scrollPositions for when the loopCount resets on a backwards scroll) twice based on certain chord indices. These indices are a bit before the end of the tab and a bit after the start of the next loop. I do the adjustments right before the start of the next loop is visible and right after the previous loop has fully left the viewport so that there is no visual jumping of the chords that the user can see. This greatly reduces the amount of re-renders and extra overhead in `<PracticeModal/>`.

**Practice playback strip animation: WAAPI vs per-chord CSS transitions**  
Previously the practice modal advanced the chord strip by updating `translateX` at each chord boundary and relying on a per-chord CSS `transition`. That made the visible motion too dependent on main-thread work such as React renders, GC, highlight updates, and slider state changes, which showed up as stutter on mobile devices and on faster tabs.

I moved the playback strip to the Web Animations API instead, generating the transform keyframes directly from the existing playback layout data (`scrollPositions` and `durations`). WAAPI was the best fit because it keeps transform animation on the compositor thread while still giving imperative control over `pause()`, `currentTime`, seeking, loop chaining, and drift correction. I explicitly did not want a pure `requestAnimationFrame` loop as the primary driver because that would put motion back on the main thread, and I did not want generated CSS `@keyframes` text because pause/seek/rate control and loop iteration handoff become much clumsier there. Audio still remains the source of truth, but scheduling is now anchored to absolute `audioContext.currentTime` targets so timing drift no longer compounds across long playback runs.
